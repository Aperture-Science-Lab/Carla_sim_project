...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            if stop_sign_found:
                # Use a COPY so we don't mutate the original waypoint
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                # Actually check if the stop sign is still in our path
                goal_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)
                self._goal_index = goal_index
                self._goal_state = waypoints[goal_index]
                # --------------------------------------------------------------
                ...
                # --------------------------------------------------------------
                if stop_sign_found:
                    # We already stopped here — remove this fence so we don't 
                    # stop at the same sign again
                    self._stopsign_fences.pop(0)
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            ...
            dist = np.linalg.norm(np.array(waypoints[wp_index+1][0:2]) - 
                                np.array(waypoints[wp_index][0:2]))
            arc_length += dist
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        dist = np.linalg.norm(np.array(waypoints[i][0:2]) - np.array(ego_state[0:2]))
        if dist < closest_len:
            closest_len = dist
            closest_index = i
    # ------------------------------------------------------------------
