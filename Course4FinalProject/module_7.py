...
def sanitize_controls(throttle, steer, brake):
    import math
    import numpy as _np
    if math.isnan(throttle) or math.isinf(throttle):
        throttle = 0.0
    if math.isnan(steer) or math.isinf(steer):
        steer = 0.0
    if math.isnan(brake) or math.isinf(brake):
        brake = 0.0
    throttle = float(_np.clip(throttle, 0.0, 1.0))
    brake = float(_np.clip(brake, 0.0, 1.0))
    steer = float(_np.clip(steer, -1.0, 1.0))
    ...
    with make_carla_client(args.host, args.port, timeout=60) as client:
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if skip_first_frame and frame == 0:
                ...
                # CRITICAL FIX: Only do math if plotting is actually enabled!
                if enable_live_plot:
                    ...
                    # set by the options
                    if live_plot_timer.has_exceeded_lap_period():
                        lp_traj.refresh()
                        lp_1d.refresh()
                        live_plot_timer.lap()
            ...
            try:
                ...
                print("Failed to send control: %s" % str(e))
                break
