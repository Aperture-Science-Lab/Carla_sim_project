        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                for k in range(len(self._circle_offsets)):
                    circle_locations[k, 0] = path[0][j] + self._circle_offsets[k] * cos(path[2][j])
                    circle_locations[k, 1] = path[1][j] + self._circle_offsets[k] * sin(path[2][j])
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            dist = np.sqrt((paths[i][0][-1] - paths[j][0][-1])**2 + (paths[i][1][-1] - paths[j][1][-1])**2)
                            score += self._weight * np.exp(-dist)
                            # --------------------------------------------------
